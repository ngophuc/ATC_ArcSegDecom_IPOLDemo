"""
Demonstration of paper:  A discrete approach for arcs and segments decomposition of noisy digital contours
...
class app(base_app):
    ...
    title = "Arcs and Segments Decomposition of Digital Contours: Online Demonstration"
    ...
    demo_src_dir = 'ATC_ArcSegDecom_IPOLDemo'
    ...
    def run(self):
        ...
        if self.cfg['meta']['original']:
            ...
            ar.add_file("outputDecomposition.png", "outputDecomposition.png", info="outputDecomposition.png")
    ...
    def run_algo(self, params):
        ...
        command_args = ['testATCArcSegDecomposition'] + \
                       [ '-i', 'inputPolygon.sdp', '-d', self.base_dir + \
                        os.path.join('srcManual/') + os.path.join('ImaGene-forIPOL')] + \
                       ['--sourceImageWidth', str(inputWidth)]+ \
                       ['--sourceImageHeight', str(inputHeight)]+ \
                       ['--maxScale', str(self.cfg['param']['maxscale'])]+ \
                       ['--samplingStep', str(self.cfg['param']['samplingstep'])]+ \
                       ['--alphaMax', str(self.cfg['param']['alphamax'])]+ \
                       ['--thickness', str(self.cfg['param']['thickness'])]+ \
        ...
        command_args = ['convert.sh', '-background', '#FFFFFF', '-flatten', \
                        'inputPolygonATC_Step4.svg', '-geometry', str(widthDisplay)+"x", 'outputATC.png']
        self.runCommand(command_args, None, fInfo)
        shutil.copy(self.work_dir + os.path.join("inputPolygonATC_Step4.svg"), 
                    self.work_dir + os.path.join("outputATC.svg"))
        ## ---------
        fInfo = open(self.work_dir+"algoLog.txt", "a")
        command_args = ['convert.sh', '-background', '#FFFFFF', '-flatten', \
                        'inputPolygon_onlyDecSA_ATC.svg','-geometry', str(widthDisplay)+"x", 'outputDecomposition.png']
        shutil.copy(self.work_dir + os.path.join("inputPolygon_onlyDecSA_ATC.svg"), 
                    self.work_dir + os.path.join("outputDecomposition.svg"))
        ...
        command_args = ['testATCArcSegDecomposition', '--version']
